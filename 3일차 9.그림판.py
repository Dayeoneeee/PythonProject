...
# 작업시(캔버스) - 그래픽을 작업할 수 있는 작업 영역
from tkinter import *
from tkinter.colorchooser import * #색상 대화 상자
from tkinter.scrolledtext import  * #입력 대화 상자(선 두께 값)
from tkinter.simpledialog import askinteger


# 선그리기 (x1, y1)-(x2, y2) : 시작과 끝 좌표 사이에 선을 그린다.
### 함수
def mouseClick(event): #마우스를 클릭시 시작 좌표 구하기
    global x1, y1
    x1 = event.x #현재 마우스 버튼을 누른 곳의 x,y 좌표를 저장(시작 좌표)
    y1 = event.y

def mouseDrop(event):  #마우스를 떼었을 때 끝좌표 구하고, 선 그리기 작업
    global x1, y1, x2, y2, penWidth, penColor
    x2 = event.x  # 현재 마우스 버튼을 누른 곳의 x,y 좌표를 저장(끝 좌표)
    y2 = event.y
    # 해당 좌표
...
def getColor(): #선택한 색상값 적용
    # 객체 변수는 자동으로 연결
    # 일반 변수는 수동으로 연결
    global penColor #전역 변수를 함수에 호출

    color = askcolor()
    penColor = color[1] #색상 이름을 pencColor에 저장
