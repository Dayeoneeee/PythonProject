...
def playGame(): #게임 구현부
    ...
    while True: #우주선, 미사일, 괴물 이동 및 동작
        ...
        for e in pygame.event.get(): #이벤트값을 읽어온다.(동시에 키 인식)
            ...
            if e.type in [pygame.KEYDOWN]: #키보드를 눌렀을 때
                if e.key == pygame.K_LEFT : dx = -5 #왼쪽 키를 눌렀을 때
            ...
            if e.type in [pygame.KEYUP]: #키보드를 떼었을 때
                if e.key == pygame.K_LEFT or e.key == pygame.K_RIGHT or\
                    ...
                    dy = dx = 0 #방향키를 떼었을 때 이동값은 0으로

        #유효성 검사(우주선이 이동할 수 있는 범위)
        #이동시 좌우범위 안에 들어가면
        if(0 < shipX+dy and shipX+dx <= swidth-shipsize[0]) \
        ...
        if missileX != None: #미사일이 발사중이면
            #몬스터 폭 안에 미사일이 들어가 있고, 몬스터 높이 안에 포함이 되면 충돌
...
def writeScore(count):
    myfont = pygame.font.SysFont("NanumGothic.ttf", 20) #사용할 글꼴
    #배경의 반대색으로 글자색으로 적용
