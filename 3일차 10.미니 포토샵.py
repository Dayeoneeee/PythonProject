# pillow 라이브러리를 이용해서 pillow에 제공하는 기능으로 간단한 이미지 편집 프로그램
# 메뉴 구성
# 파일 - 파일 열기, 파일 저장, 프로그램 종료
# 이미지 처리(1) - 확대(축소), 상하반전(좌우반전), 회전
# 이미지 처리(2) - 밝게(어둡게), 블러링, 엠보싱, 흑백 이미지
...
def func_open(): #파일 열기
    ...
    #원본이미지
    photo1 = Image.open(readFp).convert("RGB") #픽셀단위로 이미지를 변환
    oriX = photo1.width #그림의 크기를 저장
    oriY = photo1.height

    #작업을 위한 복사본
    photo2 = photo1.copy() #이미지 복사
    newX = photo2.width #작업이미지 크기
    newY = photo2.height

    displayImage(photo2, newX, newY) #작업이미지를 화면에 출력
...
def displayImage(img, width, height): #윈도우 창에 이미지 출력
    ...
    for i in range(0, height):      # 행
        tmpString = ""  # 한행의 픽셀들을 저장할 변수

        for j in range(0, width): #열
            r,g,b = rgbImage.getpixel((j, i)) #한점(픽셀)의 색상을 읽어와서 저장 r=1F, g=2F, b=37
            tmpString += "#%02x%02x%02x " %(r, g, b) #1f2f37
        print(tmpString)
...
def func_zoomout(): #확대/축소
    global window, canvas, paper, photo1, photo2, oriX, oriY
    scale = askinteger("축소", "축소할 배율(2~4)을 입력하세요.",
                        minvalue=2, maxvalue=4)
    photo2 = photo1.copy()  # 원본을 복사
    photo2 = photo2.resize((int(oriX / scale), int(oriY / scale)))
    newX = photo2.width  # 새로 변경된 크기의 이미지 사이즈 저장
...
def func_bright(): #밝게
    ...
    value = askinteger("밝게", "값(1~10)을 입력하세요?",
                        minvalue=1, maxvalue=10)  # 1은 기본 밝기
    photo2 = photo1.copy()  # 원본 복사
...
def func_dark(): #어둡게
    ...
    value = askfloat("어둡게", "값(0.0~1.0)을 입력하세요?",
                        minvalue=0.0, maxvalue=1.0) #1은 기본 밝기
    photo2 = photo1.copy()  # 원본 복사
    photo2 = ImageEnhance.Brightness(photo2).enhance(value)
    newX = photo2.width
    newY = photo2.height
...
# 이미지 읽기 -> 픽셀단위로 저장(paper) -> 효과 적용(canvas) 출력
# 모든 효과는 원본(paper)로 작업 진행
# 전역 변수
window, canvas, paper = None, None, None #윈도우, 출력용 작업지, 작용
photo, photo2 = None, None #원본 이미지, 결과 이미지
