...
# 메인메뉴
# 서브메뉴
# 메뉴바 = Menu(윈도우창) - 메뉴바를 윈도우 창에 적용
# 윈도우창.config(menu=메뉴바)
# 메인메뉴 = Menu(메뉴바) - 메인 메뉴를 메뉴바에 적용
# 메뉴바.add_cascade(label="메뉴명", menu=메인메뉴) - 메인메뉴에 항목 추가
# 메인메뉴.add_command(label="서브메뉴명", command=함수)
from tkinter import *
...
def func_exit():
    window.quit() #현재 윈도우창을 닫기
    window.destroy() #메모리 상에 윈도우 창을 제거
...
mainMenu.add_cascade(label="파일",menu=fileMenu) #파일은 fileMenu연결해서 메뉴바에 출력
